...
class FakeSiteAdapter(SiteAdapterBase):
    ...
    def manage(self):
        for k in self.runningMachines:
            if (datetime.datetime.now() - self.mr.machines[k][self.mr.regStatusLastUpdate]).seconds > random.gauss(
    ...
    def getRunningMachines(self):
        ...
        for k in self.runningMachines:
            if  self.mr.machines[k].get( self.mr.reg_status) == self.mr.StatusBooting or \
                ...
                self.mr.machines[k].get( self.mr.reg_status) == self.mr.StatusIntegrating or \
