...
def callQconf( opt, arg ):
    ret = subprocess.call(["/opt/sge6.2u5/bin/lx24-amd64/qconf", opt ,arg], env={"SGE_ROOT":"/opt/sge6.2u5","TERM":"xterm-color"})
...
def callQmod( opt, arg ):
    ret = subprocess.call(["/opt/sge6.2u5/bin/lx24-amd64/qmod", opt, "cloud.q@%s" %arg], env={"SGE_ROOT":"/opt/sge6.2u5"})
...
def makeDir( dir ):
    ...
    ret = subprocess.call(["mkdir", "-p", dir])

    if not ret == 0:
        print "error while making dir %s" %(dir)
        sys.exit(ret)
        
def mountNfs( remote_ip, remote_dir, local_dir):
    
    ps = subprocess.Popen("mount %s:%s %s" %(remote_ip, remote_dir, local_dir), bufsize=0, executable=None, stdin=None, stdout=subprocess.PIPE, shell=True)
    ret = ps.wait()
...
def callInstaller( opt, file ):
    
    ps = subprocess.Popen("./inst_sge %s -auto %s" %( opt, file ), cwd="/opt/sge6.2u5", bufsize=0, executable=None, stdin=None, stdout=subprocess.PIPE, shell=True)
    ret = ps.wait()
...
arguments = ["register_node","unregister_node","add_node","del_node","get_node_name","drain_node"]
